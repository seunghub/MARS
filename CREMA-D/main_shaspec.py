...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler, writer=None):
    ...
    for step, (spec, image, label) in enumerate(dataloader):
        ...
        target_predict, dco_predict, dco_label, m1_feature_share_cache, m2_feature_share_cache, fusion_feature = model(
...
def valid(args, model, device, dataloader):
    ...
    with torch.no_grad():
        ...
        for step, (spec, image, label) in enumerate(dataloader):
            ...
            target_predict, dco_predict, dco_label, m1_feature_share_cache, m2_feature_share_cache, fusion_feature = model(spec.unsqueeze(1).float(), image.float())
        ...
        for epoch in range(args.epochs):
            ...
            if args.use_tensorboard:
                ...
                batch_loss, batch_loss_a, batch_loss_v, a_diveristy, v_diveristy, a_re, v_re = train_epoch(args, epoch,
                                                                                                           ...
                                                                                                           scheduler)
                acc, acc_a, acc_v = valid(args, model, device, test_dataloader)
                print(f"ACC:{acc:.3f} Best ACC:{best_acc:.3f} AUDIO ACC:{acc_a:.3f} VIDEO ACC:{acc_v:.3f}")
