...
class MOE(nn.Module):
    ...
    def forward_topk(self, x, topk_idx, topk_prob):
        # x: [B,C,H,W], topk_idx: [B,K], topk_prob: [B,K]
        B, K = topk_idx.shape
        outs = []
        for b in range(B):
            acc = 0.0
            for j in range(K):
                e = int(topk_idx[b, j])
                y = self.experts[e](x[b:b+1])             
                acc = acc + topk_prob[b, j] * y
            outs.append(acc)
...
class OurDecoder(nn.Module):
    ...
    def forward(self, x, low_level_feat, x_full=None, low_level_feat_full=None):
        ...
        topk_idx = torch.topk(topk_prob, k=self.gating.top_k, dim=1).indices  # [B,K]
        topk_w   = torch.gather(topk_prob, 1, topk_idx)                        # [B,K]
        features = self.moe_layer.forward_topk(features, topk_idx, topk_w)  
